...
function geometryToGeoJson(geometry: Geometry): GeoJSON.Geometry {
  switch (geometry.type) {
    ...
    case "LineString":
      return {
        type: "LineString",
        coordinates: geometry.coordinates.map(coordinateToLngLat),
      };
    case "Polygon":
      return {
        type: "Polygon",
        coordinates: geometry.coordinates.map((ring) => ring.map(coordinateToLngLat)),
      };
  }
}
...
class MapLibreCameraAdapter implements ICameraAdapter {
  ...
  public async flyTo(viewport: Viewport): Promise<void> {
    this.map.flyTo(viewportToCamera(viewport));
  }

  public async easeTo(viewport: Viewport): Promise<void> {
    this.map.easeTo(viewportToCamera(viewport));
  }

  public async fitBounds(bounds: BoundingBox): Promise<void> {
    const value: LngLatBoundsLike = [
...
class MapLibrePopupAdapter implements IPopupAdapter {
  ...
  public async open(definition: PopupDefinition): Promise<void> {
    ...
    const popup = new maplibregl.Popup({
      closeButton: definition.closeButton ?? true,
      closeOnClick: definition.closeOnClick ?? true,
    }).setLngLat(coordinateToLngLat(definition.coordinate));

    if (typeof definition.content === "string") popup.setText(definition.content);
    else popup.setDOMContent(definition.content);
...
export class MapLibreMapAdapter implements IMapAdapter {
  ...
  public setCenter(center: Coordinate): void {
    this.map.setCenter(coordinateToLngLat(center));
  }

  public getCenter(): Coordinate {
    const center = this.map.getCenter();
    return new Coordinate(center.lng, center.lat);
  }
  ...
  public async destroy(): Promise<void> {
    await this.markers.clear();
    await this.popups.closeAll();
    this.map.remove();
  }
}
...
export class MapLibreProvider implements IMapProvider {
  ...
  public async createMap(options: CreateMapOptions): Promise<IMapAdapter> {
    ...
    const mapOptions: MapOptions = {
      ...
      ...viewportToCamera(options.viewport),
...
export function createMapLibreProvider(): MapLibreProvider {
