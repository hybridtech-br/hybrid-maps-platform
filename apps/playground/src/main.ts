...
async function bootstrap(): Promise<void> {
  ...
  app.innerHTML = `
    <main class="shell">
      <header class="topbar">
        <div><span class="eyebrow">HYBRID Tecnologia Inteligente</span><h1>Maps Platform Playground</h1></div>
        <div class="provider-badge"><span></span> MapLibre ativo</div>
      </header>
      <section class="workspace">
        <aside class="panel">
          <div class="panel-section"><label for="provider">Provider</label><select id="provider" disabled><option>MapLibre GL JS</option></select></div>
          <div class="panel-section"><h2>Capacidades</h2><ul id="capabilities" class="capabilities"></ul></div>
          <div class="panel-section actions"><h2>Controles</h2><button id="focus-rio" type="button">Centralizar no Rio</button><button id="open-popup" type="button" class="secondary">Abrir identificação</button></div>
        </aside>
        <div class="map-stage"><div id="map" aria-label="Mapa interativo do Rio de Janeiro"></div><div class="map-caption">HMP-0001 · First Running Map</div></div>
        <aside class="console-panel"><div class="console-heading"><h2>Event Console</h2><button id="clear-console" type="button" class="text-button">Limpar</button></div><ol id="event-console" class="event-console"></ol></aside>
      </section>
    </main>`;
  ...
  function logEvent(message: string): void {
    ...
    time.textContent = new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit", second: "2-digit" }).format(new Date());
  ...
  await map.markers.add({ id: "hybrid-rio", coordinate: RIO_CENTER, element: markerElement });
  ...
  async function openHybridPopup(): Promise<void> {
    ...
    await map.popups.open({ id: "hybrid-identification", coordinate: RIO_CENTER, content: popup, closeButton: true });
  ...
  document.querySelector("#clear-console")?.addEventListener("click", () => { eventConsole.replaceChildren(); logEvent("Console limpo"); });
  window.addEventListener("resize", () => map.resize());
  window.addEventListener("beforeunload", () => void provider.dispose());
}

void bootstrap().catch((error: unknown) => {
  console.error("HYBRID Maps Platform playground failed to initialize.", error);
});
